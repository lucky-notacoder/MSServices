...
function renderShowcaseList() {
  ...
  const visibleServices = [
    {
      serviceId: serviceOrder[(activeIndex - 1 + serviceOrder.length) % serviceOrder.length],
      position: "previous"
    },
    {
      serviceId: serviceOrder[(activeIndex + 1) % serviceOrder.length],
      position: "next"
    }
  ];

  visibleServices.forEach(({ serviceId, position }, index) => {
    const service = serviceDetails[serviceId];
    const card = document.createElement("button");
    card.type = "button";
    card.className = `showcase-service-option ${position === "previous" ? "is-previous" : "is-next"}`;
    card.dataset.serviceId = serviceId;
